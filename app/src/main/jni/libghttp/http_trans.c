	...
	if (a_conn->hostinfo == NULL)
	{
		...
		else
		{
			...
			if ((a_conn->hostinfo = gethostbyname(a_conn->host)) == NULL)
			{
				a_conn->error_type = http_trans_err_type_host;
				a_conn->error = h_errno;
				LOGW("%s,%d,%s", "http_trans_connect:", 3, a_conn->host);
				goto ec;
			}
		}
